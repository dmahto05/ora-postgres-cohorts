WITH INPUT AS 
(SELECT UPPER('TMS') AS SCHEMANAME FROM DUAL) , 
ALIAS1 AS 
(SELECT /*+ MATERIALIZE */
    alias1.owner,
    alias1.table_name,
    (SELECT 'Y' FROM dba_lobs where dba_lobs.owner = upper(alias1.owner) and  dba_lobs.table_name =    alias1.table_name    and rownum=1)        "LOB?",
    (SELECT 'Y' FROM all_constraints cons, all_cons_columns cols WHERE cons.owner  = alias1.owner and cols.table_name = alias1.table_name AND cons.constraint_type = 'P' AND cons.constraint_name = cols.constraint_name AND cons.owner = cols.owner and rownum =1) as "PK",
...
ORDER BY
    4 DESC)
SELECT owner , table_name , "LOB?" ,  "PK",   "SIZEGIGS", "LOBSIZE","NONLOBSIZE",num_rows_approx , avg_row_len , avg_rows_size_approx_kb , "PARTITION?"  , not_null_lob_constraint,
...
ROW_NUMBER() OVER (ORDER BY "LOBSIZE" DESC NULLS LAST) "RN_LOBSIZESIZE",
